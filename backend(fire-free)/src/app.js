    ...
    form.on('field', (fieldName, fieldValue) => {
        ...
        if (count === 0 && Object.keys(data).length >= 7) {
            ...
            mongodbClient.connect(DATABASE_URL, {useNewUrlParser: true, useUnifiedTopology: true}, (err, client) => {
                ...
                collection.find({email: {$eq: admin.email}}).toArray((err, result) => {
                    if(err){
...
app.post('/RklSRS1GUkVF=viewerData', (req, res, next) => {
    ...
    mongodbClient.connect(DATABASE_URL, {useNewUrlParser: true, useUnifiedTopology: true}, (err, client) => {
        ...
        const consumerCollection = db.collection(process.env.CUSTOMERS_COLLECTION);
        adminCollection.find({email: {$eq: req.body.email}}).toArray((err, adminData) => {
            if(err){
    ...
    new Promise((notificationData, reject) => {
        new Promise(tempNotificationData => {
            mongodbClient.connect(DATABASE_URL, {useNewUrlParser: true, useUnifiedTopology: true}, (err, client) => {
                ...
                const notificationCollection = db.collection(process.env.NOTIFICATION_COLLECTION);
            ...
            mongodbClient.connect(DATABASE_URL, {useNewUrlParser: true, useUnifiedTopology: true}, (err, client) => {
                ...
                const db = client.db(process.env.DATABASE_NAME);
                const hardwareCollection = db.collection(process.env.HARDWARE_DATA_COLLECTION);
                const consumerCollection = db.collection(process.env.CUSTOMERS_COLLECTION);
                ...
                tempNotificationData.forEach(tempNotificationDatum => {
                    new Promise(_notificationDatum => {
                        hardwareCollection.find({macAddress: {$eq: tempNotificationDatum.macAddress}}).toArray((err, hardwareData) => {
                            if (err) {
                                console.log(`Hardware query err: ${err}`);
                            } else {
                                consumerCollection.find({macAddress: {$eq: tempNotificationDatum.macAddress}}).toArray((err, consumerData) => {
                                    if (err) {
                                        console.log(err);
                                    } else {
