...
const encryption_decryption = {
    encryption: (data) => {
        ...
        const result = Buffer.concat([initializationVector, cipher.update(data), cipher.final()]);
        return result;
    },
    decryption: (data) => {
        const initializationVector = data.slice(0, 16);
        data = data.slice(16);
        const decipher = crypto.createDecipheriv(cipherAlgorithm, key, initializationVector);
