...
export class NotificationPage implements OnInit {
  ...
  async showAbout(consumerData, hardwareData, hardwareState, solvedBy) {
    if (this.clickCounter[consumerData.macAddress] && this.clickCounter[consumerData.macAddress] % 2 !== 0) {
      this.clickCounter[consumerData.macAddress] = this.clickCounter[consumerData.macAddress] + 1;
      this.clickedCard = consumerData.macAddress;
      this._isClicked.next(this.clickCounter);
    }
      ...
      <ion-row>
        <ion-col size="12">
          <ion-avatar style="position: absolute;left: 50%;right: 50%;transform: translate(-50%, -50%);margin-top: 25px;">
            <img src="${this.consumerImagePath + consumerData.imagePath}" alt="logo" width="50px" height="50px">
          </ion-avatar>
        </ion-col>
      </ion-row>
      <ion-row>
        <ion-col size="12" id="${consumerData.macAddress}">
        ...
        <p style="font-size: 12px;font-family: 'Courier New', Courier, monospace;margin-top: -20px;">
          <a href="mailto:${consumerData.email}" style="color: #94BF69">${consumerData.email}</a>
        </p>
        </ion-col>
      </ion-row>
      <ion-row>
    ...
    <div style="text-align: center;margin-bottom: -35px;">
        <p style="font-size: 12px; color: #B0B3B5;">MAC Address: ${consumerData.macAddress}</p>
    </div>`;
    if (solvedBy) {
      document.getElementById(consumerData.macAddress).innerHTML += `
      <p style="font-size: 12px;font-family: 'Courier New', Courier, monospace;margin-top: -5px;">
        Solved By:<br><a href="mailto:${solvedBy}" style="color: #94BF69">${solvedBy}</a>
      </p>`;
    }
    await about.present();
  }
