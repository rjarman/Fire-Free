...
export class MapPage implements AfterViewInit, OnDestroy {
  @ViewChild('googleMapContainer', { static: false }) googleMapContainer: ElementRef;
  ...
  private _distanceMatrixObserver = new Subject<google.maps.DistanceMatrixResponse>();
  private get distanceMatrixObserver(): Observable<google.maps.DistanceMatrixResponse> {
    return this._distanceMatrixObserver.asObservable();
  }
  distanceMatrixData: google.maps.DistanceMatrixResponse;

  private geolocationUnsubscriber: any;

  // testing
  // private fireServiceCoordinatesCounter = 0;
  // private fireServiceLAT: number;
  // private fireServiceLNG: number;
  ...
  constructor(
    ...
    this.geolocationUnsubscriber = this.geolocation.watchPosition({enableHighAccuracy: true}).subscribe((data) => {
      // console.log(`watch(lat): ${data.coords.latitude}`);
      // console.log(`watch(lng): ${data.coords.longitude}`);
      this._getFireServiceCoordinates.next(new google.maps.LatLng(data.coords.latitude, data.coords.longitude));
      if (this.isNavigate) {
        this._getFireServiceCoordinates.next(new google.maps.LatLng(data.coords.latitude + .01, data.coords.longitude));
        // console.log('isNavigate: ' + this.isNavigate);
        // console.log(this.fireServiceCoordinates);
        this.navigate();
      }
    });
  }
  ...
  ngOnDestroy() {
    this.geolocationUnsubscriber.unsubscribe();
  }

  private get setGoogleMap() {
    return new Promise((resolve, reject) => {
      this.googleMap = new google.maps.Map(this.googleMapContainer.nativeElement, this.mapOptions);
      if (this.googleMap) {
        resolve(true);
      }
    });
  ...
  private getUserPosition(positionType, tOut = Infinity) {
    return new Promise((resolve, reject) => {
      this.geolocation.getCurrentPosition({enableHighAccuracy: true, timeout: tOut, maximumAge: positionType}).then((data) => {
  ...
  private async generateInfoWindow(positionType: number = 0, isStart: boolean = false, tOut = Infinity) {
  ...
  navigate() {
    ...
    this.sliceDesOrigin();
/*
    // for testing the navigation system before attaching the h/w
    // start testing
    this._getFireServiceCoordinates.next(new google.maps.LatLng(this.fireServiceLAT + this.fireServiceCoordinatesCounter, this.fireServiceLNG));
    this.fireServiceCoordinatesCounter -= .1;
    const a = this.getDistanceMatrix;
    // end testing
*/
    ...
    this.directionsService.route({
      ...
      if (status === 'OK') {
        ...
        this.directionRenderer.setOptions({
          ...
          polylineOptions: {strokeColor: '#DD0000', strokeWeight: 7, strokeOpacity: .7}
        });
      }
    });
    ...
    setTimeout(() => {
      document.getElementById('durationInTraffic').style.color = this.trafficStyle;
    }, 1);
    setTimeout(() => {
  ...
  private sliceDesOrigin() {
    if (this.distanceMatrixData.destinationAddresses[0].length > 42) {
      this.shortDestination = this.distanceMatrixData.destinationAddresses[0].slice(0, 39) + '...';
    } else {
    ...
    if (this.distanceMatrixData.originAddresses[0].length > 42) {
      this.shortOrigin = this.distanceMatrixData.originAddresses[0].slice(0, 39) + '...';
    } else {
  ...
  private setInfoWindow() {
    ...
    if (this.distanceMatrixData.rows[0].elements[0].duration_in_traffic.value > this.distanceMatrixData.rows[0].elements[0].duration.value) {
      this.trafficStyle = 'red';
    } else {
      this.trafficStyle = 'green';
    }
    if (this.notificationDatum.solvedBy !== '' && this.notificationDatum.solvedBy !== undefined && this.notificationDatum.solvedBy !== null) {
        ...
        <ion-row>
          <ion-col size="12">
            <ion-avatar style="position: absolute;left: 50%;right: 50%;transform: translate(-50%, -50%);margin-top: 25px;">
              <img src="${this.consumerImagePath + this.notificationDatum.consumerData.imagePath}" alt="logo" width="50px" height="50px">
            </ion-avatar>
          </ion-col>
        </ion-row>
        <ion-row>
          <ion-col size="5">
            ...
            <p style="margin-top: -12px;text-align: center;">
              <strong><em>${this.distanceMatrixData.originAddresses}</em></strong>
            </p>
          </ion-col>
          ...
          <ion-col size="5">
            ...
            <p style="margin-top: -12px;text-align: center;">
              <strong><em>${this.distanceMatrixData.destinationAddresses}</em></strong>
            </p>
          </ion-col>
        </ion-row>
        <ion-row>
          <ion-col size="12">
            ...
            <p style="margin-top: -10px;text-align: center;">
              <strong style="color: red;"><em>${this.distanceMatrixData.rows[0].elements[0].distance.text}</em></strong>
            </p>
          </ion-col>
        </ion-row>
        <ion-row>
          <ion-col size="12">
            <p style="margin-top: -20px;">
              Average Arrival Time: <strong><em>${this.distanceMatrixData.rows[0].elements[0].duration.text}</em></strong>
            </p>
            <p style="margin-top: -10px;">
              In Today Traffic:
              <strong style="color: ${this.trafficStyle};">
                <em>${this.distanceMatrixData.rows[0].elements[0].duration_in_traffic.text}</em>
              </strong>
            </p>
            <p style="margin-top: -10px;">
              Solved By:
