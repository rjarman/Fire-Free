...
export class ProfilePage implements OnInit, OnDestroy {
          ...
          tempData.forEach(data => {
            Object.defineProperty(data, 'frontendStructure', {
              value: {
                ...
                shortMACAddress: data.macAddress.split('-')[0] + '-' + data.macAddress.split('-')[1] + '....'
              }
            });
          });
        }
  ...
  edit() {
    this.router.navigate(['/menus/profile/edit', JSON.stringify(this.adminData)]);
  }
  ...
  async showAbout(consumerData) {
    ...
    document.getElementsByClassName('alert-wrapper')[0]['style'].backgroundColor = '#1B5259';
  ...
  onScroll(event) {
    document.getElementById('profile').style.filter = `opacity(${OpacityStyle[event.detail.scrollTop].opacityValue})`;
    ...
    document.getElementById('stickyProfile').style.top = `${TopStyle[offsetHeight].topValue}`;
